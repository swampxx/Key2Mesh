...
def normalize_p2d(joint2d, confidence=None, threshold=0.01):
...
def align_by_pelvis(joints, left_id=3, right_id=2):
    """
    Assumes joints is 14 x 3 in common order.
    Then hips are: [3, 2]
...
def align_by_pelvis_batch(joints, left_id=3, right_id=2):
    """
    Assumes joints is B x 14 x 3 in common order.
    Then hips are: [3, 2]
    Takes mid point of these points, then subtracts it.
    """

    pelvis = (joints[:, left_id, :] + joints[:, right_id, :]) / 2.0
    return joints - pelvis.unsqueeze(1)


def compute_MPJPE(pred, real, PA=True):
    joint3d_pred = copy.deepcopy(pred)
    joint3d_real = copy.deepcopy(real)
    ...
    for j in range(joint3d_pred.shape[0]):
        ...
        if PA:
            ...
            joint3d_real_ = align_by_pelvis(joint3d_real_)
            joint3d_pred_ = align_by_pelvis(joint3d_pred_)
